...
function showMeme() {
  ...
  forImg.src = randomMemeUrl;
  parent.appendChild(forImg);
  last = 'meme';
}
...
function showJoke() {
  ...
  newNode.innerHTML = randomJokeText;
...
function showQuote() {
  ...
  const parent = document.querySelector('.content');
  const newQuote = document.createElement('p'), newAuthor = document.createElement('pre');
  newQuote.innerHTML = "'&nbsp" + randomQuote.quote + "&nbsp'";
  newAuthor.innerHTML = "&nbsp;&nbsp;-&nbsp;" + randomQuote.author;
  newAuthor.style.display = 'inline';
  newQuote.style.display = 'inline';
  parent.appendChild(newQuote);
  parent.appendChild(newAuthor);
  last = 'quote';
}
...
function showRiddle() {
  ...
  newNode.innerHTML = randomRiddle.question;
  answerRiddle.innerHTML = randomRiddle.answer;
  answerRiddle.style.display = 'none';
  parent.appendChild(newNode);
  parent.appendChild(answerRiddle);
  last = 'riddle';
}
...
function revealAnswers() {
  if(last !== "riddle") {
    if(last === 'answer') window.alert('riddle already has been answered');
    else window.alert('Fuck U, there is no riddle!!!');
    return;
  }
  document.querySelector('.content').children[1].style.display = 'block';
  last = 'answer';
}
